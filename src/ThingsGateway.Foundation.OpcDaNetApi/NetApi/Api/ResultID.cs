...
namespace Opc
{
    ...
    public struct ResultID : ISerializable
    {
        ...
        {
            m_name = new XmlQualifiedName((string)info.GetValue("NA", typeof(string)), (string)info.GetValue("NS", typeof(string)));
        ...
        public bool Succeeded()
        {
            if (Code != -1)
                return Code >= 0;
            return Name != (XmlQualifiedName)null && Name.Name.StartsWith("S_");
        }
        ...
        public bool Failed()
        {
            if (Code != -1)
                return Code < 0;
            return Name != (XmlQualifiedName)null && Name.Name.StartsWith("E_");
        }
        ...
        public override bool Equals(object target)
        {
            if (target != null && target.GetType() == typeof(ResultID))
            {
                ...
                if (resultId.Name != (XmlQualifiedName)null && Name != (XmlQualifiedName)null)
        ...
        {
            return Name != (XmlQualifiedName)null ? Name.Name : string.Format("0x{0,0:X}", (object)Code);
        }
