...
namespace Opc.Cpx
{
  public class BinaryReader : BinaryStream
  {
    public ComplexValue Read(byte[] buffer, TypeDictionary dictionary, string typeName)
    {
      ...
      Context context = this.InitializeContext(buffer, dictionary, typeName);
    ...
    private int ReadType(Context context, out ComplexValue complexValue)
    {
      ...
      for (int fieldIndex = 0; fieldIndex < type.Field.Length; ++fieldIndex)
      {
        ...
        if (this.IsArrayField(field))
          num = this.ReadArrayField(context, field, fieldIndex, fieldValues, out complexValue1.Value);
        else if (field.GetType() == typeof (TypeReference))
        {
          object fieldValue = (object) null;
          num = this.ReadField(context, (TypeReference) field, out fieldValue);
          ...
          num = this.ReadField(context, field, fieldIndex, fieldValues, out complexValue1.Value, ref bitOffset);
      ...
      out object fieldValue)
    {
      ...
      if (field.CharCountRef != null)
        length2 = this.ReadReference(context, (FieldType) field, fieldIndex, fieldValues, field.CharCountRef);
      if (length2 == -1)
      {
        length2 = 0;
      ...
      if (length1 > 2)
      {
        ...
        if (context.BigEndian)
        {
          for (int index = 0; index < bytes.Length; index += length1)
            this.SwapBytes(bytes, 0, length1);
        }
        fieldValue = (object) bytes;
      }
      else
      ...
      out object fieldValue)
    {
      ...
      if (field.ElementCountSpecified)
      {
        ...
        {
          int num = this.ReadField(context, field, fieldIndex, fieldValues, out fieldValue1, ref bitOffset);
          if (num != 0 || bitOffset != (byte) 0)
          {
      ...
      {
        int num1 = this.ReadReference(context, field, fieldIndex, fieldValues, field.ElementCountRef);
        for (int index3 = 0; index3 < num1; ++index3)
        {
          int num2 = this.ReadField(context, field, fieldIndex, fieldValues, out fieldValue1, ref bitOffset);
      ...
      else if (field.FieldTerminator != null)
      {
        ...
        for (; context.Index < context.Buffer.Length; context.Index += num)
        {
          ...
          num = this.ReadField(context, field, fieldIndex, fieldValues, out fieldValue1, ref bitOffset);
          if (num != 0 || bitOffset != (byte) 0)
            arrayList.Add(fieldValue1);
          else
            break;
