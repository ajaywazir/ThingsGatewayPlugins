...
namespace Opc.Cpx
{
  public class BinaryWriter : BinaryStream
  {
    public byte[] Write(ComplexValue namedValue, TypeDictionary dictionary, string typeName)
    {
      ...
      Context context = typeName != null ? this.InitializeContext((byte[]) null, dictionary, typeName) : throw new ArgumentNullException(nameof (typeName));
      int length = this.WriteType(context, namedValue);
      context.Buffer = length != 0 ? new byte[length] : throw new InvalidDataToWriteException("Could not write value into buffer.");
      if (this.WriteType(context, namedValue) != length)
    ...
    private int WriteType(Context context, ComplexValue namedValue)
    {
      ...
      for (int fieldIndex = 0; fieldIndex < type.Field.Length; ++fieldIndex)
      {
        ...
        int num = !this.IsArrayField(field) ? (!(field.GetType() == typeof (TypeReference)) ? this.WriteField(context, field, fieldIndex, fieldValues, fieldValue.Value, ref bitOffset) : this.WriteField(context, (TypeReference) field, (object) fieldValue)) : this.WriteArrayField(context, field, fieldIndex, fieldValues, fieldValue.Value);
    ...
    private int WriteField(Context context, TypeReference field, object fieldValue)
    {
      ...
      return !(fieldValue.GetType() != typeof (ComplexValue)) ? this.WriteType(context, (ComplexValue) fieldValue) : throw new InvalidDataToWriteException("Instance of type is not the correct type.");
    }
      ...
      object fieldValue)
    {
      ...
      if (field.CharCountRef != null)
        this.WriteReference(context, (FieldType) field, fieldIndex, fieldValues, field.CharCountRef, count);
      if (buffer != null)
      {
        if (numArray == null)
        {
        ...
        if (context.BigEndian && length > 1)
        {
          for (int index = 0; index < numArray.Length; index += length)
            this.SwapBytes(buffer, context.Index + index, length);
        }
      }
      return count * length;
    }
      ...
      object fieldValue)
    {
      ...
      if (field.ElementCountSpecified)
      {
        ...
        foreach (object fieldValue1 in array)
        {
          ...
          {
            int num2 = this.WriteField(context, field, fieldIndex, fieldValues, fieldValue1, ref bitOffset);
            if (num2 == 0)
            {
              if (bitOffset == (byte) 0)
                break;
        ...
        {
          int num3 = this.WriteField(context, field, fieldIndex, fieldValues, (object) null, ref bitOffset);
      ...
      else if (field.ElementCountRef != null)
      {
        ...
        {
          int num = this.WriteField(context, field, fieldIndex, fieldValues, fieldValue2, ref bitOffset);
          if (num == 0)
          {
            if (bitOffset == (byte) 0)
              break;
        ...
        this.WriteReference(context, field, fieldIndex, fieldValues, field.ElementCountRef, count);
      }
      else if (field.FieldTerminator != null)
      {
        foreach (object fieldValue3 in array)
        {
          int num = this.WriteField(context, field, fieldIndex, fieldValues, fieldValue3, ref bitOffset);
          if (num == 0)
          {
            if (bitOffset == (byte) 0)
              break;
        ...
        byte[] terminator = this.GetTerminator(context, field);
