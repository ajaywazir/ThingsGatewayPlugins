    ...
    public class Subscription : OpcCom.Da.Subscription
    {
        ...
        protected override ItemValueResult[] Read(ItemIdentifier[] itemIDs, Item[] items)
        {
            ...
            if (arrayList1.Count > 0)
                Read((ItemValueResult[])arrayList1.ToArray(typeof(ItemValueResult)), true);
            if (arrayList2.Count > 0)
                Read((ItemValueResult[])arrayList2.ToArray(typeof(ItemValueResult)), false);
        ...
        private void Read(ItemValueResult[] items, bool cache)
        {
            ...
            try
            {
                ((IOPCSyncIO)m_group).Read(cache ? OPCDATASOURCE.OPC_DS_CACHE : OPCDATASOURCE.OPC_DS_DEVICE, items.Length, phServer, out ppItemValues, out ppErrors);
            }
            catch (Exception ex)
            {
          ...
          out int cancelID)
        {
            try
            {
                ...
                IntPtr ppErrors = IntPtr.Zero;
                ((IOPCAsyncIO2)m_group).Read(itemIDs.Length, phServer, requestID, out cancelID, out ppErrors);
          ...
          out int cancelID)
        {
            ...
            try
            {
                ...
                for (int index = 0; index < arrayList1.Count; ++index)
                    phServer[index] = (int)((ItemIdentifier)arrayList1[index]).ServerHandle;
                IntPtr ppErrors = IntPtr.Zero;
                ((IOPCAsyncIO2)m_group).Write(arrayList1.Count, phServer, (object[])arrayList2.ToArray(typeof(object)), requestID, out cancelID, out ppErrors);
                int[] int32s = OpcCom.Interop.GetInt32s(ref ppErrors, arrayList1.Count, true);
                for (int index = 0; index < arrayList1.Count; ++index)
                {
                    IdentifiedResult identifiedResult = (IdentifiedResult)arrayList1[index];
