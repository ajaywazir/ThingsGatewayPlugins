    ...
    public class Server : OpcCom.Da.Server
    {
        ...
        public override void Initialize(URL url, ConnectData connectData)
        {
            lock (this)
            {
                ...
                try
                {
                    ...
                    ((IOPCServer)m_server).AddGroup("", 1, 0, 0, IntPtr.Zero, IntPtr.Zero, pdwLcid, out m_groupHandle, out pRevisedUpdateRate, ref guid, out m_group);
                }
                catch (Exception ex)
                {
                    Uninitialize();
        ...
        public override ItemValueResult[] Read(Item[] items)
        {
            ...
            lock (this)
            {
                ...
                IdentifiedResult[] items1 = AddItems(items);
                ...
                try
                {
                    ...
                    if (arrayList2.Count > 0)
                    {
                        try
                        {
                            ...
                            ((IOPCItemMgt)m_group).SetActiveState(arrayList2.Count, phServer, 1, out ppErrors);
                        ...
                        ReadValues((Item[])arrayList1.ToArray(typeof(Item)), (ItemValueResult[])arrayList2.ToArray(typeof(ItemValueResult)), true);
                    }
                    if (arrayList4.Count > 0)
                        ReadValues((Item[])arrayList3.ToArray(typeof(Item)), (ItemValueResult[])arrayList4.ToArray(typeof(ItemValueResult)), false);
                }
                finally
                {
                    RemoveItems(items1);
        ...
        public override IdentifiedResult[] Write(ItemValue[] items)
        {
            ...
            lock (this)
            {
                ...
                IdentifiedResult[] items2 = AddItems(items1);
                try
                {
                    ArrayList arrayList1 = new ArrayList(items.Length);
                    ArrayList arrayList2 = new ArrayList(items.Length);
                    ...
                    if (arrayList1.Count > 0)
                    {
                        ...
                        try
                        {
                            ((IOPCSyncIO)m_group).Write(arrayList1.Count, phServer, pItemValues, out ppErrors);
                        }
                        catch (Exception ex)
                        {
          ...
          out Opc.Da.BrowsePosition position)
        {
            ...
            lock (this)
            {
                ...
                {
                    BrowseElement[] elements = GetElements(arrayList.Count, itemID, filters, true, ref position1);
                    if (elements != null)
                ...
                {
                    BrowseElement[] elements = GetElements(arrayList.Count, itemID, filters, false, ref position1);
                    if (elements != null)
                        arrayList.AddRange((ICollection)elements);
                    position = (Opc.Da.BrowsePosition)position1;
                }
          ...
          bool returnValues)
        {
            ...
            if (itemIDs.Length == 0)
                return Array.Empty<ItemPropertyCollection>();
            lock (this)
            {
                if (m_server == null)
                    throw new NotConnectedException();
                ...
                for (int index = 0; index < itemIDs.Length; ++index)
                {
                    ...
                    try
                    {
                        ItemProperty[] properties2 = GetProperties(itemIDs[index].ItemName, propertyIDs, returnValues);
                        if (properties2 != null)
        ...
        private void ReadValues(Item[] items, ItemValueResult[] results, bool cache)
        {
            ...
            try
            {
                ((IOPCSyncIO)m_group).Read(cache ? OPCDATASOURCE.OPC_DS_CACHE : OPCDATASOURCE.OPC_DS_DEVICE, results.Length, phServer, out ppItemValues, out ppErrors);
            }
            catch (Exception ex)
            {
            ...
            for (int index = 0; index < results.Length; ++index)
            {
                ...
                if (results[index].Value != null)
                {
                    if (items[index].ReqType != (System.Type)null)
                    {
                        try
                        {
                            results[index].Value = ChangeType(results[index].Value, items[index].ReqType, "en-US");
                        }
                        catch (Exception ex)
                        {
        ...
        private void GetItemIDs(string itemID, ItemProperty[] properties)
        {
            try
            {
                ...
                IntPtr ppErrors = IntPtr.Zero;
                ((IOPCItemProperties)m_server).LookupItemIDs(itemID, properties.Length, pdwPropertyIDs, out ppszNewItemIDs, out ppErrors);
        ...
        private void GetValues(string itemID, ItemProperty[] properties)
        {
            try
            {
                ...
                IntPtr ppErrors = IntPtr.Zero;
                ((IOPCItemProperties)m_server).GetItemProperties(itemID, properties.Length, pdwPropertyIDs, out ppvData, out ppErrors);
        ...
        private EnumString GetEnumerator(
          ...
          bool flat)
        {
            IOPCBrowseServerAddressSpace server = (IOPCBrowseServerAddressSpace)m_server;
            if (!flat)
            {
                ...
                catch
                {
                    ...
                    catch
                    {
                        ...
                        lock (m_separatorsLock)
                            strArray = m_separators == null ? szString.Split(m_separators) : szString.Split(m_separators);
        ...
        private void DetectAndSaveSeparators(string browseName, string itemID)
        {
            ...
            lock (m_separatorsLock)
            {
                ...
                if (m_separators != null)
                {
                    ...
                    {
                        if ((int)m_separators[index] == (int)ch)
                        {
                            num = index;
                            break;
                        }
                    ...
                    if (num == -1)
                    {
                        char[] destinationArray = new char[m_separators.Length + 1];
                        Array.Copy((Array)m_separators, (Array)destinationArray, m_separators.Length);
                        m_separators = destinationArray;
                    }
                }
                if (num != -1)
                    return;
                ...
                m_separators[m_separators.Length - 1] = ch;
            }
        }
          ...
          bool isBranch)
        {
            ...
            try
            {
                ...
                ((IOPCBrowseServerAddressSpace)m_server).GetItemID(element.Name, out szItemID);
          ...
          opcitemdef
            ...
            try
            {
                if (filters.ReturnAllProperties)
                    element.Properties = GetProperties(element.ItemName, (PropertyID[])null, filters.ReturnPropertyValues);
          ...
          ref BrowsePosition position)
        {
            ...
            if (position == null)
            {
                ...
                {
                    if (branches)
                        return Array.Empty<BrowseElement>();
                    if (itemID != null && itemID.ItemName != null && itemID.ItemName.Length > 0)
                        throw new ResultIDException(ResultID.Da.E_UNKNOWN_ITEM_NAME);
                }
                enumerator = GetEnumerator(itemID?.ItemName, filters, branches, pNameSpaceType == OPCNAMESPACETYPE.OPC_NS_FLAT);
            }
            else
            ...
            do
            {
                if (strArray != null)
                {
                    for (int index = num; index < strArray.Length; ++index)
                    {
                        ...
                        BrowseElement element = GetElement(itemID, strArray[index], filters, branches);
                        if (element != null)
