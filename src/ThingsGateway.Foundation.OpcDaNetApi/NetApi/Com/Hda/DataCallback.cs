...
namespace OpcCom.Hda
{
  internal class DataCallback : IOPCHDA_DataCallback
  {
    ...
    public event CallbackExceptionEventHandler CallbackException
    {
      ...
      {
        lock (this)
          this.m_callbackException += value;
      }
      remove
      {
        lock (this)
    ...
    public bool CancelRequest(Request request, CancelCompleteEventHandler callback)
    {
      lock (this)
      {
        if (!this.m_requests.Contains((object) request.RequestID))
          return false;
        if (callback != null)
      ...
      int[] phrErrors)
    {
      try
      {
        lock (this)
        {
          Request request = (Request) this.m_requests[(object) dwTransactionID];
          if (request == null)
            return;
      ...
      {
        this.HandleException(dwTransactionID, ex);
      }
    }
      ...
      int[] phrErrors)
    {
      try
      {
        lock (this)
        {
          ...
          {
            IntPtr pInput = (IntPtr) int32s[index];
    ...
    public void OnCancelComplete(int dwCancelID)
    {
      try
      {
        lock (this)
        {
          Request request = (Request) this.m_requests[(object) dwCancelID];
          if (request == null)
            return;
      ...
      {
        this.HandleException(dwCancelID, ex);
      }
    }
    ...
    private void HandleException(int requestID, Exception exception)
    {
      lock (this)
      {
        Request request = (Request) this.m_requests[(object) requestID];
        if (request == null || this.m_callbackException == null)
          return;
        this.m_callbackException((IRequest) request, exception);
      }
    }
  }
}
