    ...
    public class Server : OpcCom.Server, Opc.Hda.IServer, Opc.IServer, IDisposable
    {
        ...
        public Opc.Hda.Attribute[] GetAttributes()
        {
            lock (this)
            {
                ...
                if (pdwCount == 0)
                    return Array.Empty<Opc.Hda.Attribute>();
        ...
        public IBrowser CreateBrowser(BrowseFilter[] filters, out ResultID[] results)
        {
            lock (this)
            {
                ...
                try
                {
                    ((IOPCHDA_Server)m_server).CreateBrowse(length, pdwAttrID, pOperator, vFilter, out pphBrowser, out ppErrors);
                }
                catch (Exception ex)
                {
            ...
            lock (this)
            {
                ...
                for (int index = 0; index < items.Length; ++index)
                {
                    if (items[index] != null)
                    {
                        ...
                        phClient[index] = Server.CreateHandle();
                    }
                }
                ...
                try
                {
                    ((IOPCHDA_Server)m_server).GetItemHandles(items.Length, pszItemID, phClient, out pphServer, out ppErrors);
                }
                catch (Exception ex)
                {
                ...
                for (int index = 0; index < items1.Length; ++index)
                {
                    ...
                    {
                        m_items.Add((object)phClient[index], (object)new ItemIdentifier()
                        {
        ...
        public IdentifiedResult[] ReleaseItems(ItemIdentifier[] items)
        {
            ...
            lock (this)
            {
                ...
                for (int index = 0; index < identifiedResultArray.Length; ++index)
                {
                    ...
                    {
                        ItemIdentifier itemIdentifier = (ItemIdentifier)m_items[items[index].ServerHandle];
                        if (itemIdentifier != null)
                        {
                            ...
                            m_items.Remove(items[index].ServerHandle);
                        }
                    }
                }
        ...
        public IdentifiedResult[] ValidateItems(ItemIdentifier[] items)
        {
            ...
            lock (this)
            {
                ...
                try
                {
                    ((IOPCHDA_Server)m_server).ValidateItemIDs(items.Length, pszItemID, out ppErrors);
                }
                catch (Exception ex)
                {
          ...
          ItemIdentifier[] items)
        {
            ...
            lock (this)
            {
                if (m_server == null)
                    throw new NotConnectedException();
                if (items.Length == 0)
                    return Array.Empty<ItemValueCollection>();
                int[] serverHandles = GetServerHandles(items);
                ...
                try
                {
                    ((IOPCHDA_SyncRead)m_server).ReadRaw(ref time1, ref time2, maxValues, includeBounds ? 1 : 0, serverHandles.Length, serverHandles, out ppItemValues, out ppErrors);
                }
                catch (Exception ex)
                {
                ...
                UpdateResults(items, (ItemIdentifier[])valueCollections, ref ppErrors);

                Server.UpdateActualTimes((IActualTime[])valueCollections, time1, time2);
          ...
          out IRequest request)
        {
            ...
            lock (this)
            {
                ...
                if (items.Length == 0)
                    return Array.Empty<IdentifiedResult>();
                Request request1 = m_callback.CreateRequest(requestHandle, (Delegate)callback);
                ...
                try
                {
                    ((IOPCHDA_AsyncRead)m_server).ReadRaw(request1.RequestID, ref time1, ref time2, maxValues, includeBounds ? 1 : 0, serverHandles.Length, serverHandles, out pdwCancelID, out ppErrors);
                }
                catch (Exception ex)
                {
                ...
                UpdateResults(items, (ItemIdentifier[])results, ref ppErrors);
                if (request1.Update(pdwCancelID, (ItemIdentifier[])results))
                {
                    request = (IRequest)null;
                    m_callback.CancelRequest(request1, (CancelCompleteEventHandler)null);
                    return results;
                }

                Server.UpdateActualTimes(new IActualTime[1]
        {
          ...
          out IRequest request)
        {
            ...
            lock (this)
            {
                ...
                try
                {
                    ((IOPCHDA_AsyncRead)m_server).AdviseRaw(request1.RequestID, ref time, filetime, serverHandles.Length, serverHandles, out pdwCancelID, out ppErrors);
                }
                catch (Exception ex)
                {
          ...
          out IRequest request)
        {
            ...
            lock (this)
            {
                ...
                try
                {
                    ((IOPCHDA_Playback)m_server).ReadRawWithUpdate(request1.RequestID, ref time1, ref time2, maxValues, filetime2, filetime1, serverHandles.Length, serverHandles, out pdwCancelID, out ppErrors);
                }
                catch (Exception ex)
                {
          ...
          Item[] items)
        {
            ...
            lock (this)
            {
                ...
                if (items.Length == 0)
                    return Array.Empty<ItemValueCollection>();
                int[] serverHandles = GetServerHandles((ItemIdentifier[])items);
                ...
                try
                {
                    ((IOPCHDA_SyncRead)m_server).ReadProcessed(ref time1, ref time2, filetime, serverHandles.Length, serverHandles, aggregateIds, out ppItemValues, out ppErrors);
                }
                catch (Exception ex)
                {
                ...
                ItemValueCollection[] valueCollections = Interop.GetItemValueCollections(ref ppItemValues, items.Length, true);
                UpdateResults((ItemIdentifier[])items, (ItemIdentifier[])valueCollections, ref ppErrors);
          ...
          out IRequest request)
        {
            ...
            lock (this)
            {
                ...
                try
                {
                    ((IOPCHDA_AsyncRead)m_server).ReadProcessed(request1.RequestID, ref time1, ref time2, filetime, serverHandles.Length, serverHandles, aggregateIds, out pdwCancelID, out ppErrors);
                }
                catch (Exception ex)
                {
                ...
                for (int index = 0; index < items.Length; ++index)
                    results[index] = new IdentifiedResult();
                UpdateResults((ItemIdentifier[])items, (ItemIdentifier[])results, ref ppErrors);
                if (request1.Update(pdwCancelID, (ItemIdentifier[])results))
                {
                    request = (IRequest)null;
                    m_callback.CancelRequest(request1, (CancelCompleteEventHandler)null);
          ...
          out IRequest request)
        {
            ...
            lock (this)
            {
                ...
                try
                {
                    ((IOPCHDA_AsyncRead)m_server).AdviseProcessed(request1.RequestID, ref time, filetime, serverHandles.Length, serverHandles, aggregateIds, numberOfIntervals, out pdwCancelID, out ppErrors);
                }
                catch (Exception ex)
                {
          ...
          out IRequest request)
        {
            ...
            lock (this)
            {
                ...
                try
                {
                    ((IOPCHDA_Playback)m_server).ReadProcessedWithUpdate(request1.RequestID, ref time1, ref time2, filetime1, numberOfIntervals, filetime2, serverHandles.Length, serverHandles, aggregateIds, out pdwCancelID, out ppErrors);
                }
                catch (Exception ex)
                {
          ...
          out IRequest request)
        {
            ...
            lock (this)
            {
                ...
                try
                {
                    ((IOPCHDA_AsyncRead)m_server).ReadAtTime(request1.RequestID, filetimEs.Length, filetimEs, serverHandles.Length, serverHandles, out pdwCancelID, out ppErrors);
                }
                catch (Exception ex)
                {
          ...
          ItemIdentifier[] items)
        {
            ...
            lock (this)
            {
                if (m_server == null)
                    throw new NotConnectedException();
                if (items.Length == 0)
                    return Array.Empty<ModifiedValueCollection>();
                int[] serverHandles = GetServerHandles(items);
                OPCHDA_TIME time1 = Interop.GetTime(startTime);
                OPCHDA_TIME time2 = Interop.GetTime(endTime);
                IntPtr ppItemValues = IntPtr.Zero;
                ...
                try
                {
                    ((IOPCHDA_SyncRead)m_server).ReadModified(ref time1, ref time2, maxValues, serverHandles.Length, serverHandles, out ppItemValues, out ppErrors);
                }
                catch (Exception ex)
                {
          ...
          out IRequest request)
        {
            ...
            lock (this)
            {
                ...
                try
                {
                    ((IOPCHDA_AsyncRead)m_server).ReadModified(request1.RequestID, ref time1, ref time2, maxValues, serverHandles.Length, serverHandles, out pdwCancelID, out ppErrors);
                }
                catch (Exception ex)
                {
          ...
          int[] attributeIDs)
        {
          ...
          item
                ...
                try
                {
                    ((IOPCHDA_SyncRead)m_server).ReadAttribute(ref time1, ref time2, serverHandles[0], attributeIDs.Length, attributeIDs, out ppAttributeValues, out ppErrors);
                }
                catch (Exception ex)
                {
                ...
                ItemAttributeCollection attributeCollection = UpdateResults(item, valueCollections, ref ppErrors);
          ...
          out IRequest request)
        {
          ...
          item
                ...
                try
                {
                    ((IOPCHDA_AsyncRead)m_server).ReadAttribute(request1.RequestID, ref time1, ref time2, serverHandles[0], attributeIDs.Length, attributeIDs, out pdwCancelID, out ppErrors);
                }
                catch (Exception ex)
                {
                ...
                UpdateResult(item, (ItemIdentifier)result1, 0);
          ...
          ItemIdentifier[] items)
        {
            ...
            lock (this)
            {
                if (m_server == null)
                    throw new NotConnectedException();
                if (items.Length == 0)
                    return Array.Empty<AnnotationValueCollection>();
                int[] serverHandles = GetServerHandles(items);
                OPCHDA_TIME time1 = Interop.GetTime(startTime);
                OPCHDA_TIME time2 = Interop.GetTime(endTime);
                ...
                try
                {
                    ((IOPCHDA_SyncAnnotations)m_server).Read(ref time1, ref time2, serverHandles.Length, serverHandles, out ppAnnotationValues, out ppErrors);
                }
                catch (Exception ex)
                {
          ...
          out IRequest request)
        {
            ...
            lock (this)
            {
                ...
                try
                {
                    ((IOPCHDA_AsyncAnnotations)m_server).Read(request1.RequestID, ref time1, ref time2, serverHandles.Length, serverHandles, out pdwCancelID, out ppErrors);
                }
                catch (Exception ex)
                {
        ...
        public ResultCollection[] InsertAnnotations(AnnotationValueCollection[] items)
        {
            ...
            lock (this)
            {
                if (m_server == null)
                    throw new NotConnectedException();
                if (items.Length == 0)
                    return Array.Empty<ResultCollection>();
                ResultCollection[] resultCollections = CreateResultCollections((ItemIdentifier[])items);
                ...
                int count = MarshalAnnotatations(items, ref serverHandles, ref ftTimestamps, ref annotations);
                if (count == 0)
                ...
                Server.UpdateResults((ICollection[])items, resultCollections, count, ref ppErrors);
                return resultCollections;
            }
        }
          ...
          out IRequest request)
        {
            ...
            lock (this)
            {
                ...
                int count = MarshalAnnotatations(items, ref serverHandles, ref ftTimestamps, ref annotations);
                if (count == 0)
                    return GetIdentifiedResults(resultCollections);
                Request request1 = m_callback.CreateRequest(requestHandle, (Delegate)callback);
                IntPtr ppErrors = IntPtr.Zero;
                int pdwCancelID = 0;
                try
                {
                    ((IOPCHDA_AsyncAnnotations)m_server).Insert(request1.RequestID, serverHandles.Length, serverHandles, ftTimestamps, annotations, out pdwCancelID, out ppErrors);
                }
                catch (Exception ex)
                {
        ...
        public ResultCollection[] Insert(ItemValueCollection[] items, bool replace)
        {
            ...
            lock (this)
            {
                ...
                int count = MarshalValues(items, ref handles, ref values, ref qualities, ref timestamps);
                if (count == 0)
                    return resultCollections;
                OPCHDA_FILETIME[] filetimEs = Interop.GetFILETIMEs(timestamps);
                IntPtr ppErrors = IntPtr.Zero;
          ...
          out IRequest request)
        {
            ...
            lock (this)
            {
                ...
                if (replace)
                {
                    try
                    {
                        ((IOPCHDA_AsyncUpdate)m_server).InsertReplace(request1.RequestID, handles.Length, handles, filetimEs, values, qualities, out pdwCancelID, out ppErrors);
                    }
                    catch (Exception ex)
                    {
                ...
                else
                {
                    try
                    {
                        ((IOPCHDA_AsyncUpdate)m_server).Insert(request1.RequestID, handles.Length, handles, filetimEs, values, qualities, out pdwCancelID, out ppErrors);
                    }
                    catch (Exception ex)
                    {
          ...
          out IRequest request)
        {
            ...
            lock (this)
            {
                ...
                try
                {
                    ((IOPCHDA_AsyncUpdate)m_server).Replace(request1.RequestID, handles.Length, handles, filetimEs, values, qualities, out pdwCancelID, out ppErrors);
                }
                catch (Exception ex)
                {
        ...
        public IdentifiedResult[] Delete(Time startTime, Time endTime, ItemIdentifier[] items)
        {
            ...
            lock (this)
            {
                ...
                try
                {
                    ((IOPCHDA_SyncUpdate)m_server).DeleteRaw(ref time1, ref time2, serverHandles.Length, serverHandles, out ppErrors);
                }
                catch (Exception ex)
                {
          ...
          out IRequest request)
        {
            ...
            lock (this)
            {
                ...
                try
                {
                    ((IOPCHDA_AsyncUpdate)m_server).DeleteRaw(request1.RequestID, ref time1, ref time2, serverHandles.Length, serverHandles, out pdwCancelID, out ppErrors);
                }
                catch (Exception ex)
                {
        ...
        public ResultCollection[] DeleteAtTime(ItemTimeCollection[] items)
        {
            ...
            lock (this)
            {
                ...
                int[] handles = (int[])null;
                DateTime[] timestamps = (DateTime[])null;
                int count = MarshalTimestamps(items, ref handles, ref timestamps);
                if (count == 0)
                    return resultCollections;
                OPCHDA_FILETIME[] filetimEs = Interop.GetFILETIMEs(timestamps);
                IntPtr ppErrors = IntPtr.Zero;
          ...
          out IRequest request)
        {
            ...
            lock (this)
            {
                ...
                try
                {
                    ((IOPCHDA_AsyncUpdate)m_server).DeleteAtTime(request1.RequestID, handles.Length, handles, filetimEs, out pdwCancelID, out ppErrors);
                }
                catch (Exception ex)
                {
        ...
        {
            CancelRequest(request, (CancelCompleteEventHandler)null);
        }
        ...
        public void CancelRequest(IRequest request, CancelCompleteEventHandler callback)
        {
            ...
            lock (this)
            {
                ...
                m_callback.CancelRequest(request1, callback);
                try
                {
                    ((IOPCHDA_AsyncRead)m_server).Cancel(request1.CancelID);
                }
                catch (Exception ex)
                {
        ...
        private static int CreateHandle() => Server.NextHandle++;
        ...
        private static int GetCount(ICollection[] collections)
        {
            int count = 0;
            if (collections != null)
            {
        ...
        private ResultCollection[] CreateResultCollections(ItemIdentifier[] items)
        {
            ...
            if (items != null)
            {
                ...
                for (int index = 0; index < items.Length; ++index)
                {
                    ...
                    if (items[index] != null)
                        UpdateResult(items[index], (ItemIdentifier)resultCollections[index], 0);
                }
            }
            return resultCollections;
        }
        ...
        private static int[] GetAggregateIDs(Item[] items)
        {
        ...
        private static void UpdateActualTimes(
          IActualTime[] results,
          ...
          ref IntPtr pErrors)
        {
            ...
            UpdateResult(item, (ItemIdentifier)result, 0);
            return result;
        }
          ...
          ref IntPtr pErrors)
        {
            ...
            {
                UpdateResult(items[index], results[index], int32s[index]);
          ...
          ref DateTime[] timestamps)
        {
            ...
            int count = Server.GetCount((ICollection[])items);
          ...
          ref OPCHDA_ANNOTATION[] annotations)
        {
            ...
            int count = Server.GetCount((ICollection[])items);
            int[] serverHandles1 = GetServerHandles((ItemIdentifier[])items);
        ...
        private static IdentifiedResult[] GetIdentifiedResults(ResultCollection[] results)
        {
