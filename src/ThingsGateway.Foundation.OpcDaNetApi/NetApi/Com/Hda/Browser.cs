...
namespace OpcCom.Hda
{
  public class Browser : IBrowser, IDisposable
  {
      ...
      ResultID[] results)
    {
      ...
      this.m_filters = new BrowseFilterCollection((ICollection) arrayList);
    }

    ~Browser() => this.Dispose(false);
    ...
    protected virtual void Dispose(bool disposing)
    {
      ...
      lock (this)
      {
        ...
        OpcCom.Interop.ReleaseServer((object) this.m_browser);
        this.m_browser = (IOPCHDA_Browser) null;
      }
    ...
    public BrowseElement[] Browse(ItemIdentifier itemID)
    {
      ...
      BrowseElement[] browseElementArray = this.Browse(itemID, 0, out position);
      if (position == null)
      ...
      out IBrowsePosition position)
    {
      ...
      lock (this)
      {
        ...
        try
        {
          this.m_browser.ChangeBrowsePosition(OPCHDA_BROWSEDIRECTION.OPCHDA_BROWSE_DIRECT, itemName);
        }
        catch (Exception ex)
        {
          throw OpcCom.Interop.CreateException("IOPCHDA_Browser.ChangeBrowsePosition", ex);
        }
        EnumString enumerator1 = this.GetEnumerator(true);
        ArrayList arrayList = this.FetchElements(enumerator1, maxElements, true);
        ...
        enumerator1.Dispose();
        EnumString enumerator2 = this.GetEnumerator(false);
        ArrayList c = this.FetchElements(enumerator2, maxElements - arrayList.Count, false);
        if (c != null)
    ...
    public BrowseElement[] BrowseNext(int maxElements, ref IBrowsePosition position)
    {
      ...
      lock (this)
      {
        ...
        {
          arrayList = this.FetchElements(browsePosition.Enumerator, maxElements, true);
          if (arrayList.Count >= maxElements)
            return (BrowseElement[]) arrayList.ToArray(typeof (BrowseElement));
          ...
          try
          {
            this.m_browser.ChangeBrowsePosition(OPCHDA_BROWSEDIRECTION.OPCHDA_BROWSE_DIRECT, browsePosition.BranchPath);
          }
          catch (Exception ex)
          {
            throw OpcCom.Interop.CreateException("IOPCHDA_Browser.ChangeBrowsePosition", ex);
          }
          browsePosition.Enumerator = this.GetEnumerator(false);
        }
        ArrayList c = this.FetchElements(browsePosition.Enumerator, maxElements - arrayList.Count, false);
        if (c != null)
          arrayList.AddRange((ICollection) c);
        if (arrayList.Count >= maxElements)
          return (BrowseElement[]) arrayList.ToArray(typeof (BrowseElement));
    ...
    private string GetFullBranchName(string name)
    {
      ...
      try
      {
        this.m_browser.ChangeBrowsePosition(OPCHDA_BROWSEDIRECTION.OPCHDA_BROWSE_DOWN, name);
      }
      catch
      {
        return (string) null;
      ...
      try
      {
        this.m_browser.GetBranchPosition(out pszBranchPos);
      }
      catch
      {
      }
      this.m_browser.ChangeBrowsePosition(OPCHDA_BROWSEDIRECTION.OPCHDA_BROWSE_UP, "");
      return pszBranchPos;
    }
    ...
    private ArrayList FetchElements(EnumString enumerator, int maxElements, bool isBranch)
    {
      ...
      while (arrayList.Count < maxElements)
      {
        ...
        if (strArray != null && strArray.Length != 0)
        {
          foreach (string str in strArray)
          {
            ...
            {
              if (isBranch)
                pszItemID = this.GetFullBranchName(str);
              else
      ...
      IdentifiedResult[] identifiedResultArray = this.m_server.ValidateItems((ItemIdentifier[]) arrayList.ToArray(typeof (ItemIdentifier)));
