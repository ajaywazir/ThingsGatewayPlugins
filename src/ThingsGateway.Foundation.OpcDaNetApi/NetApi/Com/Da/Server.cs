    ...
    public class Server : OpcCom.Server, Opc.Da.IServer, Opc.IServer, IDisposable
    {
                ...
                lock (this)
                {
                    if (disposing && m_server != null)
                    {
                        foreach (Subscription subscription in (IEnumerable)m_subscriptions.Values)
                        {
                            ...
                            try
                            {
                                ((IOPCServer)m_server).RemoveGroup((int)subscription.GetState().ServerHandle, 0);
                            }
                            catch
                            {
                            }
        ...
        public virtual ItemValueResult[] Read(Item[] items)
        {
            ...
            lock (this)
            {
                ...
                try
                {
                    ((IOPCItemIO)m_server).Read(length, pszItemIDs, pdwMaxAge, out ppvValues, out ppwQualities, out ppftTimeStamps, out ppErrors);
                }
                catch (Exception ex)
                {
                ...
                for (int index = 0; index < itemValueResultArray.Length; ++index)
                {
                    ...
                    if (itemValueResultArray[index].Value != null)
                    {
                        if (items[index].ReqType != (System.Type)null)
                        {
                            try
                            {
                                itemValueResultArray[index].Value = ChangeType(varianTs[index], items[index].ReqType, locale);
                            }
                            catch (Exception ex)
                            {
        ...
        public virtual IdentifiedResult[] Write(ItemValue[] items)
        {
            ...
            lock (this)
            {
                ...
                try
                {
                    ((IOPCItemIO)m_server).WriteVQT(length, pszItemIDs, opcitemvqTs, out ppErrors);
                }
                catch (Exception ex)
                {
                    throw OpcCom.Interop.CreateException("IOPCItemIO.Read", ex);
        ...
        public ISubscription CreateSubscription(SubscriptionState state)
        {
            ...
            lock (this)
            {
                ...
                try
                {
                    ((IOPCServer)m_server).AddGroup(state1.Name != null ? state1.Name : "", state1.Active ? 1 : 0, state1.UpdateRate, 0, IntPtr.Zero, gcHandle.AddrOfPinnedObject(), OpcCom.Interop.GetLocale(state1.Locale), out phServerGroup, out pRevisedUpdateRate, ref guid, out ppUnk);
                }
                catch (Exception ex)
                {
                ...
                Subscription subscription = CreateSubscription(ppUnk, state1, m_filters);
                m_subscriptions[(object)phServerGroup] = (object)subscription;
        ...
        public void CancelSubscription(ISubscription subscription)
        {
            ...
            lock (this)
            {
                ...
                if (!m_subscriptions.ContainsKey(subscriptionState.ServerHandle))
                    throw new ArgumentException("Handle not found.", nameof(subscription));
                m_subscriptions.Remove(subscriptionState.ServerHandle);
                subscription.Dispose();
                try
                {
                    ((IOPCServer)m_server).RemoveGroup((int)subscriptionState.ServerHandle, 0);
                }
                catch (Exception ex)
                {
          ...
          out Opc.Da.BrowsePosition position)
        {
            ...
            lock (this)
            {
                ...
                if (pbMoreElements != 0 || !string.IsNullOrEmpty(stringUni))
                    position = (Opc.Da.BrowsePosition)new BrowsePosition(itemID, filters, stringUni);

                Server.ProcessResults(browseElements, filters.PropertyIDs);
        ...
        public virtual BrowseElement[] BrowseNext(ref Opc.Da.BrowsePosition position)
        {
            lock (this)
            {
                ...
                if (browsePosition == null || string.IsNullOrEmpty(browsePosition.ContinuationPoint))
                    throw new BrowseCannotContinueException();
                int pdwCount = 0;
                int pbMoreElements = 0;
                ...
                if (pbMoreElements == 0 && string.IsNullOrEmpty(browsePosition.ContinuationPoint))
                    position = (Opc.Da.BrowsePosition)null;
          ...
          bool returnValues)
        {
            ...
            lock (this)
            {
                ...
                try
                {
                    ((IOPCBrowse)m_server).GetProperties(itemIDs.Length, pszItemIDs, returnValues ? 1 : 0, propertyIDs != null ? propertyIDs.Length : 0, Interop.GetPropertyIDs(propertyIDs), out ppItemProperties);
                }
                catch (Exception ex)
                {
