...
namespace OpcCom.Da.Wrapper
{
      ...
      IOPCWrappedServer
    {
        ...
        protected Server() => RegisterInterface(typeof(IOPCShutdown).GUID);
        ...
        public int SetGroupName(string oldName, string newName)
        {
            lock (this)
            {
                Group group = (Group)m_groups[(object)oldName];
                if (newName == null || newName.Length == 0 || group == null)
                    return -2147024809;
                if (m_groups.Contains((object)newName))
                    return -1073479668;
                m_groups.Remove((object)oldName);
                m_groups[(object)newName] = (object)group;
                return 0;
            }
        }
        ...
        internal Group CreateGroup(ref SubscriptionState state, int lcid, int timebias)
        {
            lock (this)
            {
                ...
                if (m_groups.Contains((object)state.Name))
                {
                ...
                m_groups[(object)state.Name] = (object)group;
                return group;
            }
        }
        ...
        public void GetGroupByName(string szName, ref Guid riid, out object ppUnk)
        {
            lock (this)
            {
                try
                {
                    foreach (Group group in (IEnumerable)m_groups.Values)
                    {
            ...
            lock (this)
            {
                try
                {
                    foreach (Group group in (IEnumerable)m_groups.Values)
                    {
                        ...
                        {
                            m_groups.Remove((object)group.Name);
                            group.Dispose();
                            return;
                        }
                    }
        ...
        public void CreateGroupEnumerator(OPCENUMSCOPE dwScope, ref Guid riid, out object ppUnk)
        {
            lock (this)
            {
                try
                {
                    ...
                    {
                        ppUnk = (object)new EnumUnknown((ICollection)m_groups);
                    }
                    else
                    {
                        if (!(riid == typeof(IEnumString).GUID))
                            throw new ExternalException("E_NOINTERFACE", -2147467262);
                        ArrayList strings = new ArrayList(m_groups.Count);
                        foreach (Group group in (IEnumerable)m_groups.Values)
        ...
        public void GetStatus(out IntPtr ppServerStatus)
        {
            lock (this)
            {
                try
                {
                    OPCSERVERSTATUS serverStatus = OpcCom.Da.Interop.GetServerStatus(m_server.GetStatus(), m_groups.Count);
          ...
          out IntPtr ppBrowseElements)
        {
            lock (this)
            {
                try
                {
                    ...
                    {
                        input = m_server.Browse(itemID, filters, out position);
                    }
                    else
                    {
                        Server.ContinuationPoint continuationPoint = (Server.ContinuationPoint)m_continuationPoints[(object)key];
                        if (continuationPoint != null)
                        {
                            position = continuationPoint.Position;
                            m_continuationPoints.Remove((object)key);
                        }
                        if (position == null)
                        ...
                        input = m_server.BrowseNext(ref position);
                    }
                    CleanupContinuationPoints();
                    if (position != null)
                    {
                        string str = Guid.NewGuid().ToString();
                        m_continuationPoints[(object)str] = (object)new Server.ContinuationPoint(position);
        ...
        public void GetItemID(string szItemDataID, out string szItemID)
        {
            lock (this)
            {
                try
                {
                    ...
                    else if (IsItem(szItemDataID))
                        szItemID = szItemDataID;
                    else
                        szItemID = (FindChild(szItemDataID) ?? throw Server.CreateException(-2147024809)).ItemName;
                }
                catch (Exception ex)
                {
                    throw Server.CreateException(ex);
                ...
                try
                {
                    ...
                    switch (dwBrowseDirection)
                    {
                        case OPCBROWSEDIRECTION.OPC_BROWSE_UP:
                            ItemIdentifier itemID1 = m_browseStack.Count > 0 ? (ItemIdentifier)m_browseStack.Pop() : throw Server.CreateException(-2147467259);
                            if (m_browseStack.Count > 0 && m_browseStack.Peek() == null)
                            {
                                BuildBrowseStack(itemID1);
                                break;
                            }
                            break;
                        case OPCBROWSEDIRECTION.OPC_BROWSE_DOWN:
                            BrowseElement browseElement = szString != null && szString.Length != 0 ? FindChild(szString) : throw Server.CreateException(-2147024809);
                            if (browseElement == null || !browseElement.HasChildren)
                                throw Server.CreateException(-2147024809);
                            m_browseStack.Push((object)new ItemIdentifier(browseElement.ItemName));
                            break;
                        case OPCBROWSEDIRECTION.OPC_BROWSE_TO:
                            ...
                            try
                            {
                                browseElementArray = m_server.Browse(itemID2, filters, out position);
                            }
                            catch
                            {
                                throw Server.CreateException(-2147024809);
                            ...
                            if (browseElementArray == null || browseElementArray.Length == 0)
                                throw Server.CreateException(-2147024809);
                            m_browseStack.Clear();
                            m_browseStack.Push((object)null);
                            m_browseStack.Push((object)itemID2);
                            break;
                    }
                    position?.Dispose();
                }
          ...
          out IntPtr ppErrors)
        {
            ...
            new ItemIdentifier(szItemID)
                    ...
                    ItemPropertyCollection[] properties = m_server.GetProperties(itemIDs, propertyIDs, false);
          ...
          out IntPtr ppvtDataTypes)
        {
            lock (this)
            {
                try
                {
                    ItemPropertyCollection[] propertyCollectionArray = szItemID != null && szItemID.Length != 0 ? m_server.GetProperties(new ItemIdentifier[1]
                    {
            new ItemIdentifier(szItemID)
          ...
          out IntPtr ppErrors)
        {
            ...
            new ItemIdentifier(szItemID)
                    ...
                    for (int index = 0; index < propertyIDs.Length; ++index)
                        propertyIDs[index] = OpcCom.Da.Interop.GetPropertyID(pdwPropertyIDs[index]);
                    ItemPropertyCollection[] properties = m_server.GetProperties(itemIDs, propertyIDs, true);
                    if (properties == null || properties.Length != 1)
                        throw Server.CreateException(-2147467259);
                ...
                if (dwCount != 0 && pszItemIDs != null)
                {
                    if (pItemVQT != null)
                    {
                        try
                        {
                            ...
                            IdentifiedResult[] results = m_server.Write(values);
        ...
        {
            ArrayList arrayList = new ArrayList();
            foreach (DictionaryEntry continuationPoint in m_continuationPoints)
            {
                try
                {
        ...
        private BrowseElement FindChild(string name)
        {
            ...
            try
            {
                ...
                filters.ReturnPropertyValues = false;
                Opc.Da.BrowsePosition position = (Opc.Da.BrowsePosition)null;
                browseElementArray = m_server.Browse(itemID, filters, out position);
                position?.Dispose();
            }
            catch
            {
        ...
        {
            m_browseStack.Clear();
            BuildBrowseStack((ItemIdentifier)null, itemID);
        }
        ...
        private bool BuildBrowseStack(ItemIdentifier itemID, ItemIdentifier targetID)
        {
            ...
            foreach (BrowseElement browseElement in browseElementArray)
            {
                ...
                if (targetID.ItemName.StartsWith(browseElement.ItemName))
                {
                    ...
                    m_browseStack.Push((object)itemID1);
                    return BuildBrowseStack(itemID1, targetID);
                }
            }
            return false;
        }
        ...
        private sealed class ContinuationPoint
        {
