  ...
  public class EnumOPCItemAttributes : IEnumOPCItemAttributes
  {
    ...
    internal EnumOPCItemAttributes(ICollection items)
    {
      ...
      foreach (EnumOPCItemAttributes.ItemAttributes itemAttributes in (IEnumerable) items)
        this.m_items.Add((object) itemAttributes);
    }
    ...
    public void Clone(out IEnumOPCItemAttributes ppEnumItemAttributes)
    {
      lock (this)
      {
        try
        {
          ppEnumItemAttributes = (IEnumOPCItemAttributes) new EnumOPCItemAttributes((ICollection) this.m_items);
        }
        catch (Exception ex)
        {
          throw Server.CreateException(ex);
    ...
    public void Next(int celt, out IntPtr ppItemArray, out int pceltFetched)
    {
      lock (this)
      {
        try
        {
          ...
          {
            EnumOPCItemAttributes.ItemAttributes itemAttributes = (EnumOPCItemAttributes.ItemAttributes) this.m_items[this.m_index + index];
            ...
            ptr = (IntPtr) (ptr.ToInt64() + (long) Marshal.SizeOf(typeof (OPCITEMATTRIBUTES)));
          }
