  ...
  public class Interop
  {
    ...
    internal static OPCSERVERSTATUS GetServerStatus(ServerStatus input, int groupCount)
    {
      ...
      if (input != null)
      {
        ...
        {
          string[] strArray = input.ProductVersion.Split(new char[1]
          {
            '.'
          });
      ...
      bool deallocate)
    {
      ...
      if (pInput != IntPtr.Zero && count > 0)
      {
        ...
        for (int index = 0; index < count; ++index)
        {
          ...
          ptr = (IntPtr) (ptr.ToInt64() + (long) Marshal.SizeOf(typeof (OPCITEMPROPERTIES)));
        }
        if (deallocate)
        {
          Marshal.FreeCoTaskMem(pInput);
    ...
    internal static ItemValue[] GetItemValues(ref IntPtr pInput, int count, bool deallocate)
    {
      ...
      if (pInput != IntPtr.Zero && count > 0)
      {
        ...
        for (int index = 0; index < count; ++index)
        {
          ...
          ptr = (IntPtr) (ptr.ToInt64() + (long) Marshal.SizeOf(typeof (OPCITEMSTATE)));
        }
        if (deallocate)
        {
          Marshal.FreeCoTaskMem(pInput);
